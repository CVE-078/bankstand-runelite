...
**Architecture:** A new opt-in (`configManager` key, panel checkbox, default OFF) gates quest capture. When on, the existing `@Schedule` capture reads all `Quest.values()` states on the client thread (alongside the skill read, one consistent snapshot) into a `Map<String,String>` of `Quest.name()` -> `QuestState.name()`, gated by a new `QuestBaseline` (only submit on change), and appends a `quests` block to the envelope. Reuses `submitSnapshotWithRetry`, and advances the quest baseline only when the server confirms a store (`res.isStored()`) so an un-stored block keeps retrying and self-heals when the server side lands. Server side (Slice A, #407) accepts, gates, and per-user-merges the block ONCE #407 is merged and `PLUGIN_QUESTS_INGEST_ENABLED` is on; until then `main` silently strips the unknown `quests` key, so quests are dropped and the quest baseline simply does not advance (no false ack). A per-capability stored-ack (#415) would close the remaining split-flag window.
